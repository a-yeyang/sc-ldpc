...
| SC 码率 | 基图 | RL 特征 | RL 逐边 | CEM | 随机搜索 | round-robin | 默认 seed0 | 门限增益(seed0−优化) |
|---:|:--:|---:|---:|---:|---:|---:|---:|---:|
| 0.479 | BG2 | 0.156 | 0.109 | 0.110 | **0.091** | 0.625 | 0.517 | **0.49 dB** |
| 0.603 | BG2 | 0.098 | 0.112 | 0.098 | **0.095** | 0.644 | 0.709 | **0.92 dB** |
| 0.692 | BG1 | 0.008 | 0.007 | 0.007 | **0.005** | 0.012 | 0.074 | 0.17 dB |
| 0.770 | BG1 | 0.360 | 0.385 | **0.279** | 0.386 | 0.499 | 0.696 | 0.14 dB |
| 0.906 | BG1 | **0.164** | 0.176 | 0.203 | 0.186 | 0.313 | 0.318 | 0.12 dB |

**结论：**
1. **所有优化器在全部 5 个码率上都大幅优于仓库默认 seed0 与朴素 round-robin**（优化器 0.005–0.39
   vs 基线 0.31–0.71），瀑布门限增益 **0.12–0.92 dB**，并且**在中码率 R≈0.6 处最大（~0.9 dB）、
   向高码率单调缩小**——与"阈值饱和填补 BP–MAP 门限差、而该差在高码率很小"的理论（README §5.3）
   **完全一致**。这是最硬的结论：**构造优化在 0.5–0.9 全程都有用，但收益随码率递减**。
2. **诚实点：在这个大预算（1152 次评估）下，四种优化器彼此相当**——RL 特征 / 逐边 / CEM / 随机搜索
   的单点 FER、瀑布门限都在伯仲之间（R0.8 上 CEM 0.279 最好、R0.5/0.6/0.7 随机搜索略优、R0.9
   RL 特征略优）。原因正如 §5.2：**best-of-1152 的随机搜索本身就是强基线**，大预算把 RL 的
   "样本效率"优势抹平了。**RL 相对随机搜索的价值在小预算（§5.2 学习曲线）、可迁移（§5.4）、
   可解释（§5.3）三点，而非大预算单点 FER 上碾压。**
3. **机理随码率迁移**：低/中码率由"避免 4 环"主导（与 §5.3 一致，优化器把 n4 压到 0 或近 0）；
   **高码率下 4 环与性能解耦**（R0.7 的 round-robin n4=3264 却不差、R0.9 各构造 n4 高低与 FER 无关）
   ——高码率的主导有害结构已不再是 4 环。

> 一句话：**边扩展构造优化在 0.5–0.9 全程都把误码压到默认/朴素构造的几分之一（门限增益最高 ~0.9 dB
> @ R≈0.6，随码率递减）；在大评估预算下 RL、CEM、随机搜索相当，RL 的独特价值是小预算样本效率 +
> 可迁移 + 可解释。**
