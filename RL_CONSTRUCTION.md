...
### 5.7 长码 + 高码率 + 大 w：RL 的"主场"（集群 2×80 核）
...
评估的小预算 → **样本效率成为硬通货**；搜索空间 `(w+1)^E`，E=69–172，**w=3 时高达 4¹⁷²**，随机搜索
根本覆盖不了。

**结果（FER@工作点，2×80 核各约 76 分钟；数据 `results_big_{A,B}.json` / 表 `results_big_finals.csv`）：**

| 码率 | w | E | SNR | RL特征 | RL逐边 | CEM | 随机搜索 | 默认seed0 | RL vs 随机 |
|---:|--:|--:|--:|---:|---:|---:|---:|---:|:--:|
| 1/2 | 1 | 172 | 1.5 | 0.913 | 0.900 | 0.910 | 0.907 | 0.993 | ✓(微) |
...
**结论：**
1. **RL（特征或逐边）在 13/15 个组合上优于同预算随机搜索**——与 §5.2/§5.6 中短码"大预算下打平"形成
   鲜明对比。换到"贵评估 + 大空间"，RL 就赢了。
2. **优势随耦合记忆 w 增大而扩大**：w=1 → 4/5，w=2 → 4/5，**w=3 → 5/5 全胜**，且常常 **RL 把 FER 直接
   压到 0、而随机搜索还卡在 0.05–0.14**（如 R=2/3 w=3：RL **0.000** vs 随机 0.140；R=7/8 w=3：**0.000**
   vs 0.050）。**w 越大、空间 (w+1)^E 越大，随机搜索越无能、RL 的结构先验越值钱。**
3. RL **全面碾压仓库默认随机构造**（如 R=1/2 w=2：0.017 vs 0.707；R=2/3 w=3：0.000 vs 0.773）。
4. **诚实**：2 个例外（R=5/6 w=1、R=7/8 w=2）都在 w=1/2（空间小、随机够用）；R=1/2 w=1 的自动工作点偏低
   （FER~0.9，区分度差），其余工作点信号正常。

> 一句话：**这组实验证实了 RL 的"主场"——长码（评估贵）+ 大 w（空间大）下，RL 明显且系统地优于
> 随机搜索/CEM，优势随搜索空间单调增大。这正是中短码大预算下被抹平、而在此处被放大的"样本效率 +
> 结构先验"红利。**

> 图 `exp_big_rl_vs_random.svg`（RL/随机优势比 vs 码率，按 w 分线）、每 cell 的 `exp_big_ber_R*_w*.svg`、
> 链长 L-sweep `exp_big_lsweep_*.svg`；**训练 loss 曲线数据** `results_big_hist.csv`（每 cell×方法×评估次数
> 的 mean_reward 与 best_val_fer）。

### 5.7.1 把 w 推到 12：贵评估下 RL 优势随 w 单调放大（蒙特卡洛，20 格全跑完）
...
| 码率 | w=3 | w=5 | w=8 | w=12 |
|---:|---|---|---|---|
| 2/3 | RL 0.017 / 随机 0.040 | 0.010 / 0.083 | **0.000 / 0.133** | —(无信号) |
| 3/4 | 0.007 / 0.033 | 0.010 / 0.010 | **0.000 / 0.010** | 0.003 / 0.087 |
| 5/6 | 0.007 / 0.063 | 0.007 / 0.143 | **0.000 / 0.203** | —(无信号) |
| 7/8 | 0.003 / 0.100 | 0.003 / 0.023 | **0.000 / 0.043** | 0.010 / 0.037 |
| 1/2 | 0.000 / 0.037 | —(无信号) | —(无信号) | —(无信号) |

**结论：**
1. **RL 优势随 w 单调放大**：看 R=2/3 与 5/6——w 从 3→8，**随机搜索越来越差**（0.040→0.083→0.133；
   0.063→0.143→0.203），而 **RL 越来越好、常直接到 0**（0.017→0.010→**0.000**；0.007→0.007→**0.000**）。
   w 越大、搜索空间 `(w+1)^E` 越大，随机搜索越无能、RL 的样本效率+结构先验越值钱。
2. **RL 胜率随 w**：w=3 → 5/5，w=5 → 3/4，**w=8 → 4/4，w=12 → 2/2**（仅统计有信号格；非胜的都是平、无一负）。
3. **默认构造几乎全 0.7–1.0**（完全崩）——长码大 w 下"不优化构造"直接不可用。
4. **诚实**：4 个"无信号"格（R=1/2 的 w=5/8/12、R=2/3 的 w=12）所有方法 FER=1.0——是**大 w 的耦合码率
   损失**把 SC 码率拉得过低（如 R=1/2 w=12 → SC 0.427）、自动工作点落到瀑布以下所致，非解码 bug
   （同样 w=8 在 R=2/3 就正常）。更鲁棒的工作点选择或换 DE/PEXIT 门限目标可修复。

> 这把 §5.7（w≤3，RL 13/15）延伸到 w≤12，**把"RL 优势随评估成本 × 空间大小放大"这条规律钉死**。
> 图 `exp_wmc_rl_vs_random.svg`；数据 `results_wmc_ALL.json`（20 格，含训练 hist）。

### 5.8 大耦合记忆 w（3–30）：一条"何时该用 RL"的清晰边界
...
| RL 逐边 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| 随机搜索 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| 默认 seed0 | 0 | 960 | 912 | 928 | 928 | 0 | 0 |

**结论（一条清晰的边界）：**
1. **优化（RL 或随机搜索）在全 w 范围都把 4 环压到 0**；而单次默认构造（seed0）不可靠（w=5–16 卡在
   ~900）。**"优化 vs 不优化"的价值确凿、与 w 无关。**
2. **但在"廉价代理 + 大预算"下，RL ≈ 随机搜索**（都到 0）——这恰好**反衬出 RL 的真正边界**：评估**贵**时
   （§5.7 长码 Monte-Carlo，RL 13/15 胜、w 越大优势越大），样本效率让 RL 赢；评估**便宜**时（这里的
   4 环代理），随机搜索 1400 次暴力就能达到同样的 n4=0，RL 没有额外优势。
3. **何时该用 RL**：**目标函数昂贵**（长码 Monte-Carlo、密度演化/PEXIT 门限）时——样本效率是硬通货；
   目标**便宜**（短环计数）时，直接随机搜索/CEM 即可，无需 RL。

> 一句话：**RL 的价值随"单次评估成本"单调上升。** 这把 §5.2/§5.6（中短码大预算，RL≈随机）、
> §5.7（长码贵评估，RL 13/15 胜）、§5.8（大 w 廉价代理，RL≈随机）三段结果统一成一条规律——
> **RL 不是万能，但在"评估贵 + 空间大"的真实码设计场景里，它系统性地优于盲搜。**

> 图 `exp_wl_n4_vs_w.svg`（n4 vs w，按方法分线）、`exp_wl_n4_vs_L_w*.svg`（n4 vs 链长 L）；
> 数据 `results_wl.json`（含每 w 的训练 hist）。
