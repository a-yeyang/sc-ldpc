...
\begin{tikzpicture}[every node/.style={font=\small}]
  \def\bs{0.52} % block size
  % ----- (a) edge spreading: colour each base edge by its component ----------
  \begin{scope}[shift={(0,0)}]
    \node[font=\small] at (2*\bs,1.15) {\textbf{(a)} Base protograph $B$};
    \node[font=\footnotesize,text=black!70] at (2*\bs,0.78) {(one component code)};
    \foreach \r in {0,1,2}\foreach \c in {0,1,2,3}
      \draw[gray!55] (\c*\bs,-\r*\bs) rectangle +(\bs,-\bs);
    ...
    \node[font=\footnotesize] at (2*\bs,-3.5*\bs) {$B=B_0+B_1+B_2$};
  \end{scope}
  % ----- arrow: edge spreading -----------------------------------------------
  \draw[-{Stealth[length=3mm]},very thick,black!75] (2.45,-0.78) -- (4.05,-0.78);
  \node[font=\small,align=center] at (3.25,-0.42) {edge};
  \node[font=\small,align=center] at (3.25,-0.72) {spreading};
  \node[font=\footnotesize,text=black!70,align=center] at (3.25,-1.18) {(stagger by\\component)};
  % ----- (b) band-diagonal coupled matrix H_SC (staircase) -------------------
  \begin{scope}[shift={(4.7,0)}]
    \node[font=\small] at (2*\bs,1.15) {\textbf{(b)} Coupled $H_{\mathrm{SC}}$ (band-diagonal)};
    \foreach \r in {0,...,5}\foreach \c in {0,...,3}
      \draw[gray!22] (\c*\bs,-\r*\bs) rectangle +(\bs,-\bs);
    \foreach \t in {0,1,2,3}{
      \foreach \off/\col in {0/compZero,1/compOne,2/compTwo}{
        \pgfmathtruncatemacro{\row}{\t+\off}
        \fill[\col] (\t*\bs,-\row*\bs) rectangle +(\bs,-\bs);
        \draw[gray!45] (\t*\bs,-\row*\bs) rectangle +(\bs,-\bs);
      }
    }
    \node[font=\footnotesize,below] at (2*\bs,-6*\bs-0.08) {variable positions $t=0,\dots,L{-}1$};
    \node[font=\footnotesize,rotate=90,anchor=south] at (-0.32,-3*\bs) {check positions $i$};
    % termination brace + label, to the RIGHT (full width gives room)
    \draw[thick,black!55] (4*\bs+0.07,-4*\bs) -- (4*\bs+0.18,-4*\bs)
       -- (4*\bs+0.18,-6*\bs) -- (4*\bs+0.07,-6*\bs);
    \node[font=\footnotesize,right,text=black!70] at (4*\bs+0.24,-5*\bs) {termination};
  \end{scope}
  % ----- legend (under panel (a), clear of panel (b)'s axis labels) ----------
  \begin{scope}[shift={(0.15,-2.5)}]
    \fill[compZero](0,0)rectangle+(0.26,0.26);\node[right,font=\footnotesize]at(0.30,0.13){$B_0$};
    \fill[compOne](0.95,0)rectangle+(0.26,0.26);\node[right,font=\footnotesize]at(1.25,0.13){$B_1$};
    \fill[compTwo](1.9,0)rectangle+(0.26,0.26);\node[right,font=\footnotesize]at(2.20,0.13){$B_2$};
  \end{scope}
\end{tikzpicture}
\caption{Edge spreading and the resulting spatially-coupled code. \textbf{(a)} A
single base protograph $B$ is split into $w{+}1$ components by assigning each base
edge to one of $B_0,B_1,B_2$ (a colouring). \textbf{(b)} Staggering the components
along the diagonal yields the band-diagonal coupled matrix
$H_{\mathrm{SC}}[i,t]=B_{i-t}$ over coupling length $L$; the last $w$ check positions
are the termination. The colouring in (a)---which edge goes to which
component---is the construction variable this paper optimizes; in standard
pipelines it is drawn from a random number generator.}
\label{fig:scstruct}
\end{figure*}
