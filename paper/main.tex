...
%                     Sec. IV-D.  Stub only.
%   TODO-4 (LEMMA-num): numerical verification of the same-column<->4-cycle
%                     lemma via count_4cycles() across Z in {16,32,64}
%                     (measured 930 -> 0 cross-check).  Sec. V-C.
...
learned $8$-parameter feature policy reduces to an interpretable rule---spread
the edges of the same base column across distinct components---which removes the
dominant population of $4$-cycles (girth $4\!\to\!6$, $n_4\!\approx\!930\!\to\!0$);
we state a combinatorial lemma linking same-column co-assignment to countable
$4$-cycles in the lifted quasi-cyclic graph, and outline how finite-length
protograph EXIT (PEXIT) analysis explains the rate dependence of the waterfall
gain via threshold saturation. \textbf{(C3)} We give a deployable criterion for
...
optimize it end-to-end by policy gradient; the learned $8$-parameter policy is
an interpretable ``spread same-column edges'' rule that removes $4$-cycles. We
state a combinatorial lemma connecting same-column co-assignment to countable
$4$-cycles in the lifted QC graph (Section~\ref{sec:lemma}) and outline a
...
policy learn structural rules such as ``spread same-column edges.''
...
%==============================================================================
\section{A Combinatorial Lemma:\\ Same-Column Co-Assignment vs.\ 4-Cycles}
\label{sec:lemma}
%==============================================================================
We now make precise the rule that the feature policy learns
(Section~\ref{sec:results-mech}), turning the empirical observation ``spread
same-column edges'' into a statement about $4$-cycles in the lifted QC graph.

\subsection{Lemma Statement}
\begin{quote}
\emph{Lemma (informal).} Let an SC-LDPC code be obtained from a 5G-NR base graph
by edge spreading according to an assignment, and then QC-lifted by circulant
shifts $P^{s}$ ($Z\times Z$ circulants). Suppose two systematic base edges
$e_i,e_j$ of the same variable base column are assigned to the same component
$B_a$ and share the same check base row. Then, after lifting, the pair produces
a countable number of length-$4$ cycles between the corresponding (variable
block, check block) pair, governed by the standard QC $4$-cycle
condition~\cite{fossorier2004qc}. Assigning $e_i,e_j$ to distinct components
$B_a \ne B_b$ places them at \emph{different} check-position blocks of the coupled
graph, breaking that $4$-cycle. Consequently, a construction that maximizes
within-column component diversity drives the per-column $4$-cycle count to a
lower bound of zero.
\end{quote}

\subsection{Proof Sketch}
A length-$4$ cycle in a QC Tanner graph passes through two variable blocks and
two check blocks connected by four circulant edges. For two systematic edges in
the same base column assigned to the same component and incident to the same
check base row, the lifted circulants are co-located in the same block of
$H_{\mathrm{SC}}$; the standard QC condition of~\cite{fossorier2004qc} then
counts the resulting short cycles through the differences of the circulant
shifts (a $\gcd$-type count), which is generically positive. Spreading the two
edges to different components $B_a \ne B_b$ relocates one of them to a different
check-position block ($t{+}a$ vs.\ $t{+}b$) of the coupled band-diagonal
structure, so the two edges no longer share both endpoints' blocks and the
$4$-cycle is destroyed. This is the same selective-cycle-avoidance principle
formalized by the ACE metric~\cite{tian2004ace}, here specialized to the coupled
edge-spreading degree of freedom. Maximizing within-column component diversity
therefore minimizes the count of these dominant same-column $4$-cycles.

\subsection{Numerical Validation (Pending)}\label{sec:lemma-num}
% TODO-4 (LEMMA-num)
The lemma's prediction is to be cross-checked against the measured $4$-cycle
counts using the repository's \texttt{count\_4cycles} and \texttt{girth}
routines, comparing the round-robin / seed-0 counts ($\approx\!930$) to the
optimized count ($0$), and verifying the scaling across $Z\in\{16,32,64\}$.
\TODO{run numerical lemma check (count\_4cycles cross-check, measured
$930\!\to\!0$, across $Z$).} This numerical confirmation is consistent with the
mechanism measurements in Section~\ref{sec:results-mech}, but the dedicated
lemma-validation script has not yet been run.
...
\subsection{Mechanism: RL Rediscovers ``Avoid 4-Cycles''}
...
Table~\ref{tab:theta}. Read out, they encode an interpretable rule: \emph{spread
the systematic edges of the same base column across distinct components and keep
global load balanced} (strongly negative col-frac / glob-frac weights, strongly
positive col-empty weight). This is exactly the same-column $4$-cycle avoidance
of Section~\ref{sec:lemma}, rediscovered \emph{from the end-to-end FER reward
alone}, with no hand-coded cycle / trapping-set knowledge.

\begin{table}[t]
\centering
\caption{Learned Feature-Policy Weights (the Construction Rule).}
\label{tab:theta}
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{3pt}
\scriptsize
...
value is threefold: it \emph{rediscovers an interpretable rule} from the FER
reward alone, it is \emph{more sample-efficient} than same-budget random search,
and its feature policy \emph{transfers zero-shot}.
...
gain $0.12$--$0.92$\,dB, largest at $R\!\approx\!0.6$), and rediscovers an
interpretable, zero-shot-transferable ``spread same-column edges'' rule whose
mechanism is the elimination of $4$-cycles. Beyond the codes themselves, the
